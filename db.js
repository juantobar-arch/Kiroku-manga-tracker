    ...
    this.autoIncrement = {
      ...
      anime:
        this.data.anime.length > 0
          ...
          : 1,
      user_anime:
        this.data.user_anime.length > 0
          ? Math.max(...this.data.user_anime.map((ua) => ua.id)) + 1
          : 1,
    };
  }
    ...
    return {
      run: async (...params) => {
        ...
        if (sqlLower.includes("insert into user_anime")) {
          ...
          const [user_id, anime_id, status, current_episode, rating, notes] =
            params;

          if (
            this.data.user_anime.some(
              (ua) => ua.user_id === user_id && ua.anime_id === anime_id
            )
          ) {
            throw new Error("UNIQUE constraint failed");
          }
        ...
        if (sqlLower.includes("update user_anime")) {
          ...
          const index = this.data.user_anime.findIndex(
            (ua) => ua.id === parseInt(id) && ua.user_id === user_id
          );
        ...
        if (sqlLower.includes("delete from user_anime")) {
          ...
          this.data.user_anime = this.data.user_anime.filter(
            (ua) => !(ua.id === parseInt(id) && ua.user_id === user_id)
          );
      ...
      all: async (...params) => {
        ...
        if (sqlLower.includes("from user_anime ua")) {
          ...
          let results = this.data.user_anime.filter(
