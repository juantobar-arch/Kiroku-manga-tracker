...
router.post("/auth/register", async (req, res) => {
    ...
    try {
        ...
        const result = await stmt.run(email, hashedPassword, username || email.split('@')[0]);
        ...
        res.status(201).json({ 
            ...
            token,
            user: { id: result.lastInsertRowid, email, username: username || email.split('@')[0] }
        });
    } catch (error) {
...
router.post("/user/anime", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const result = await stmt.run(req.user.id, anime_id, status || 'plan_to_watch', current_episode || 0, rating, notes);
...
router.put("/user/anime/:id", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const result = await stmt.run(status, current_episode, rating, notes, id, req.user.id);
...
router.delete("/user/anime/:id", authenticateToken, async (req, res) => {
    ...
    try {
        const stmt = db.prepare("DELETE FROM user_anime WHERE id = ? AND user_id = ?");
...
router.get("/jikan/search", async (req, res) => {
    ...
    try {
        const response = await fetch(`${JIKAN_API}/anime?q=${q || ''}&page=${page}&limit=${limit}&order_by=popularity&sort=asc`);
...
router.get("/jikan/top", async (req, res) => {
    ...
    try {
        const response = await fetch(`${JIKAN_API}/top/anime?page=${page}&limit=${limit}`);
        const data = await response.json();
