...
class UI {
  ...
  static createAnimeCard(jikanAnime, onClick) {
    ...
    const genres =
      jikanAnime.genres?.map((g) => g.name).join(", ") || "Sin género";
    ...
    card.innerHTML = `
            ...
            <div class="anime-card__info">
                ...
                ${
                  jikanAnime.score
                    ? `<p class="anime-card__rating">⭐ ${jikanAnime.score}</p>`
                    : ""
                }
            </div>
        `;
  ...
  static createWatchlistItem(item, onUpdate, onDelete) {
    ...
    div.className =
      "flex items-center gap-4 hover:bg-neutral-800 rounded-lg px-4 py-3 transition-colors duration-200 justify-between";

    div.innerHTML = `
            <div class="flex items-center gap-4">
                <div class="bg-center bg-no-repeat aspect-[2/3] bg-cover rounded-md w-12" 
                     style="background-image: url('${
                       item.cover_image || "https://via.placeholder.com/300x450"
                     }');"></div>
                <div class="flex flex-col justify-center">
                    ...
                    <p class="text-neutral-400 text-sm font-normal leading-normal line-clamp-2">
                        Episodio ${item.current_episode} de ${
      item.total_episodes || "?"
    }
                    </p>
                </div>
